...
def sysrom():
    while True:
        ...
        if biosinput == 1:
            ...
            print("PCI/E 3 : None")
            print("PCI 4 : None")
            print("PCI 5 : Sound BLASTER! 16")
            print("PCI 6 : None")
            print("ISA 7 : None")
            print("Note that most SDESK 300VS Models come with 6 physical slots which means that PCI 6 and ISA 7 can NOT be used at the same time. Current SDESK 300VS has 7 physical slots.")
            print("Only 7 of those can be used with physical cards. The others are connected via a chipset bridge or are allocated to PCI lanes. PCI/E devices are connected to the chipset bridge while standard devices are connected using PCI. Legacy devices can be activated and will run in ISA Mode")
            ...
            print("Integrated sound : Disabled : Sound BLASTER! in PCI 5")
...
def copyfiles(directory,fromm):
    ...
    clearscreen()
    useless = IntVar()
    l11 = Label(windows,text="Setup Options",bg="#016699")
    r1 = Radiobutton(windows,text="Typical : Recommended for this installation",variable=useless,value=1,bg="#016699")
    r2 = Radiobutton(windows,text="Portable : Windows 98 will be setup with components useful for portable computers",variable=useless,value=2,bg="#016699")
    r3 = Radiobutton(windows,text="Compact : To save disk space, no optional components will be installed.",variable=useless,value=3,bg="#016699")
    r4 = Radiobutton(windows,text="Custom : This option is for advanced users and administrators only. Use if you want to customize your Windows installation.",variable=useless,value=4,bg="#016699")
    useless = useless.get()
    b1 = Button(windows,text="Continue",bg="#016699",command=lambda: realcopyfiles(directory,fromm))
    l11.pack(anchor=W)
    r1.pack(anchor=W)
    r2.pack(anchor=W)
    r3.pack(anchor=W)
    r4.pack(anchor=W)
    b1.pack()
def realcopyfiles(directory,fromm):
...
def realsetup():
    ...
    while True:
        try:
            ...
            x = messagebox.askretrycancel("Error","Please insert the correct CD-ROM in the CD-ROM drive. If you do not have access to it, please acquire it before running SETUP.")
            if x != True:
                   try:
                        otherloc = filedialog.askdirectory()
                        os.chdir(otherloc)
